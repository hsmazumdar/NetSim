...
namespace AstBfsObs
{
    ...
    {
        //**********************************************************
        PointF[] nods; //nodes (x,y) array
        float[] nodSz; //nodes renaining power as size array
        ArrayList nod2dNew;
        ArrayList closeNods;
        ...
        private void DoAStarBsf(bool bstFst)
        {
            ...
            for (; ; )
            {
                string[] st20 = (string[])nod2dNew[curNo];//neighbors st20[mxnod][3]
                int[] srcLst20 = Str2IntArr(st20[0].ToString().Split(','));//neighbors no
                int[] srcLst21 = Str2IntArr(st20[1].ToString().Split(','));//neighbors distance
                int nbrMx = int.Parse(st20[2]);//max neighbors
                int iMn = -1;
                int iMn2 = -1;
                float fCostMin = float.MaxValue;// minimum cost 
                for (int i2 = 0; i2 < nbrMx; i2++)//neighbors loop
                {
                    int ng1 = srcLst20[i2];//neighbors no
                    int ng = GetNg(ng1, closeNods);//if ng1 is in closeNods list
                    if (ng >= 0)
                    {
                        int nowNo = (int)closeNods[closeNods.Count - 1];//nowNo is furthest in closeNods
